...
Modified by Fernanda Perez June 2025.
------------------------------------------------------------------------- */
                                 ...
                                 const std::string& fragid)
{
    ...
    if (fragid == "all") {
      ...
      for (int i = 0; i < onemol->natoms; i++) {
        if (onemol->fragmentmask[ifrag][i]) {
          ...
          if (nsum == 0) {
            iatom1 = iatom;
            sumvvec += vvec[iatom][ivar]; //erased by Fernanda June 11, 2025
            nsum++;
          }
          else if (nsum==1) {
            iatom2 = iatom;
  ...
  //     // ALL THIS NEW, Fernanda June 11, 2025 - Removed for collagen sims Billie September 2025
      // if (nsum != 2)
      //   error->one(FLERR,"Bond/react: Molecule fragment of rxndiffIvan must contain exactly two atoms");
      // // coordinates
      // double *x1 = atom->x[iatom1];
      // double *x2 = atom->x[iatom2];
      // // compute displacement in periodic-aware way
      // double dx[3];
      // for (int idim=0; idim<3; idim++) {
      //   dx[idim] = x1[idim] - x2[idim];
      // }
      // // apply minimum image in x and y
      // if (domain->xperiodic)
      //   dx[0] -= domain->prd[0] * rint(dx[0]/domain->prd[0]);
      // if (domain->yperiodic)
      //   dx[1] -= domain->prd[1] * rint(dx[1]/domain->prd[1]);
      // z is non-periodic
      // assign result
     // sumvvec = dx[ivar];
    ...
    for (int j = 0; j < twomol->natoms; j++) {
      ...
      if (!create_atoms[j][rxnID] && !delete_atoms[ipre][rxnID]) {
        ...
        if (fit_incr == 0) {    
          ...
          double u1 = random[rxnID]->uniform();
          double ang = (float)(2 * M_PI * random[rxnID]->uniform());
          double BoxSizeX = (domain->boxhi[0] - domain->boxlo[0]);
          double BoxSizeY = (domain->boxhi[1] - domain->boxlo[1]);
          //printf("My box size is  (L = %.2f) \n", (domain->boxhi[0] - domain->boxlo[0]));
          double R = 8*(BoxSizeX+BoxSizeY)/60;
          if (fit_incr == 0) { 
              xfrozen[fit_incr][0] = (BoxSizeX)*0.5 - 3 + R*u1*cos(ang)+domain->boxlo[0]; 
              xfrozen[fit_incr][1] = (BoxSizeY)*0.5 + R*u1*sin(ang)+ domain->boxlo[1]; 
              xfrozen[fit_incr][2] = 31.5;
              }
          else { // positive in X only
                xfrozen[fit_incr][0] = xfrozen[0][0] + fit_incr;
                xfrozen[fit_incr][1] = xfrozen[0][1];
          ...
          //if (fit_incr == 0) {                          // 1st template particle, define random position :D Use individual reaction random number generator random[rxnID] - Sample normal distribution in Y (with standard deviation modify_create_nucrand[rxnID]) for new position -- Chris 28/07/2023
            ...
            //double u2 = random[rxnID]->uniform();
            //if (s > 0){
        //(domain->boxhi[0] - domain->boxlo[0]) * (random[rxnID]->uniform()-0.5);
                //ang = M_PI;
                //phi = M_PI*0.5;
                //s = 1.0;
            //}
